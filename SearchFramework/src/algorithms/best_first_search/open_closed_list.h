...
protected:
     ...
     */
    virtual bool defaultNodeNoWorse(const BFSNode<state_t, action_t> &node_1,
            const BFSNode<state_t, action_t> &node_2) const;

    /**
     * Returns true if the evaluation of the first node is less than or equal to the second
     * using High-G for tie breaking
     *
     * @param node_1 The first node being compared.
     * @param node_2 The second node being compared.
     * @return If the evaluation of the first node is less than or equal to the second.
     */
    virtual bool highGNodeNoWorse(const BFSNode<state_t, action_t> &node_1,
            const BFSNode<state_t, action_t> &node_2) const;

    /**
     * Returns true if the evaluation of the first node is less than or equal to the second
     * using Low-G for tie breaking
     *
     * @param node_1 The first node being compared.
     * @param node_2 The second node being compared.
     * @return If the evaluation of the first node is less than or equal to the second.
     */
    virtual bool lowGNodeNoWorse(const BFSNode<state_t, action_t> &node_1,
            const BFSNode<state_t, action_t> &node_2) const;
...
bool OpenClosedList<state_t, action_t>::heapifyUp(BFSOpenLocation loc)
{
    ...
    BFSNode<state_t, action_t> node_1 = getNode(open_list_heap[parent_loc]); 
    BFSNode<state_t, action_t> node_2 = getNode(open_list_heap[loc]);
...
template<class state_t, class action_t>
inline bool OpenClosedList<state_t, action_t>::defaultNodeNoWorse(const BFSNode<state_t, action_t>& node_1,
        const BFSNode<state_t, action_t>& node_2) const
{
    // printf("cost: %f\n", node_1.eval);
...
template<class state_t, class action_t>
inline bool OpenClosedList<state_t, action_t>::highGNodeNoWorse(const BFSNode<state_t, action_t>& node_1,
        const BFSNode<state_t, action_t>& node_2) const
{
    if (fp_equal(node_1.eval, node_2.eval)) {
        return !fp_less(node_1.g_cost, node_2.g_cost);
    } else {
        return !fp_greater(node_1.eval, node_2.eval);
    }
}
...
template<class state_t, class action_t>
inline bool OpenClosedList<state_t, action_t>::lowGNodeNoWorse(const BFSNode<state_t, action_t>& node_1,
        const BFSNode<state_t, action_t>& node_2) const
{
    if (fp_equal(node_1.eval, node_2.eval)) {
        return fp_less(node_1.g_cost, node_2.g_cost);
